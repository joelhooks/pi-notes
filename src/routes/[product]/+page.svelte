...
<main>
  <header class="topbar compact">
    <a class="crumb" href="/notes">Notes</a>
    {#if documents.length}
  ...
  <section class="layout">
            ...
            {#each paraSections as section}
              <section class="para-section">
                ...
                {#each section.entries as entry}
                  <a class="entry-card" href={`/${data.product}/${entry.id}`}>
          ...
          <article class="kit-email">
            ...
            <div class="email-body">
              {#each document.blocks as block}
                {@const blockId = `${document.id}:${block.id}`}
                <button
                  type="button"
                  class="block {selectedIds.has(blockId) ? 'selected' : ''} {block.kind === 'heading' ? 'subject' : ''} {block.kind === 'code' ? 'code-block' : ''}"
                  aria-pressed={selectedIds.has(blockId)}
                  onclick={(event) => toggleBlock({ documentId: document.id, sourcePath: document.sourcePath, blockId, blockKind: block.kind, text: block.text, hash: block.hash }, event)}
                >
                  <span class="comment">select</span>
                  <span class="block-copy">{@html block.html}</span>
                </button>
              {/each}
            </div>
          </article>
    ...
    <aside class="sidebar">
      ...
      <section class="lane-section roundtrip">
        ...
        {#if sessionState.session?.url}<p><a href={sessionState.session.url}>{sessionState.session.url}</a></p>{/if}
      </section>
...
<style>
  ...
  .block-copy :global(h1) { font-size: 24px; }
  .block-copy :global(h2) { font-size: 20px; }
  .block-copy :global(h3) { font-size: 17px; }
  .block-copy :global(ul), .block-copy :global(ol) { margin: 0; padding-left: 22px; display: grid; gap: 5px; white-space: normal; }
  .block-copy :global(li) { padding-left: 2px; line-height: 1.45; }
  .block-copy :global(code) { border-radius: 5px; padding: 1px 5px; background: #f1f5f9; color: #1f2937; font-size: .92em; }
  .block-copy :global(pre) { margin: 0; white-space: pre; font-size: 12px; border-radius: 8px; padding: 14px; overflow: auto; max-height: none; line-height: 1.55; }
  .block-copy :global(pre code) { background: transparent; color: inherit; padding: 0; }
  .block-copy :global(a) { color: #0f5ec7; text-decoration: underline; text-underline-offset: 3px; font-weight: 600; }
  .block-copy :global(.excalidraw-preview) { margin: 4px 0; border: 1px solid #e4e4e0; border-radius: 10px; background: #fdfdfb; overflow: hidden; }
  .block-copy :global(.excalidraw-preview figcaption) { padding: 7px 10px; border-bottom: 1px solid #ececea; color: #6b7280; font-size: 12px; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; white-space: normal; }
  .block-copy :global(.diagram-warning) { padding: 8px 10px; border-top: 1px solid #ececea; color: #7c2d12; background: #fff7ed; font-size: 12px; line-height: 1.45; }
  .block-copy :global(.diagram-warning code) { color: #7c2d12; background: #ffedd5; }
  .block-copy :global(.excalidraw-viewport) { width: 100%; max-height: 72vh; overflow: hidden; background: #f7f7f5; container-type: inline-size; }
  .block-copy :global(.excalidraw-canvas) { position: relative; background: #f7f7f5; transform: scale(min(1, calc(100cqw / (var(--diagram-width) * 1px)))); transform-origin: top left; }
  .block-copy :global(.excalidraw-shape) { position: absolute; border: 1.5px solid #24292f; border-radius: 10px; box-sizing: border-box; }
  .block-copy :global(.excalidraw-shape.ellipse) { border-radius: 999px; }
  .block-copy :global(.excalidraw-shape.diamond) { transform: rotate(45deg); transform-origin: center; }
  .block-copy :global(.excalidraw-text) { position: absolute; color: #24292f; text-align: center; line-height: 1.2; white-space: pre-wrap; pointer-events: none; }
  .block-copy :global(.excalidraw-arrow) { position: absolute; overflow: visible; pointer-events: none; }
  .subject { margin: 0; padding: 10px 5px 5px; }
  .code-block { background: #fbfbfa; border-color: #eeeeeb; }
  ...
  .roundtrip a { color: #3b4652; text-decoration: underline; text-underline-offset: 3px; }
